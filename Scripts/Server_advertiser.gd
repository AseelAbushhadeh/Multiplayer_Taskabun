...
export (float) var broadcast_interval = .3
...
func broadcast():
	#server_info.current_players=(Persistent_nodes.get_child_count()-2)/2
	var count=get_tree().multiplayer.get_network_connected_peers()
	server_info.current_players=count.size()+1
